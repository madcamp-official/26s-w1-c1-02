/* 끝말잇기 (word-chain-game) — 레벨제(vowel-game.js와 동일 20레벨 패턴) + 혼자 타임어택.
   레벨 = 전체 게임시간(레벨이 오를수록 짧아짐) 안에 살아남으면 클리어.
   시계는 전체 시간 하나뿐 — "턴"은 시간이 아니라 개수(성공한 단어 수)로만 센다.
   오답/중복/보스조건 미달이면 목숨 1개 차감(같은 턴 재도전), 목숨이 다 떨어지면 종료.
   5번째 턴마다 보스 턴(5글자 이상). 검증/두음법칙/사전 조회는 백엔드 /api/games/wordchain.
  ...
  window.WordChainGame = {
    mount(container, opts = {}) {
      ...
      function showLevelSelect() {
        ...
        container.innerHTML = `
          <div class="vg-levels">
            ...
            <div class="vg-hint">전체 시간 안에 살아남으면 클리어 · 목숨 ${LIVES_START}개, 오답이어도 같은 턴 다시 도전 가능</div>
          </div>`;
        ...
        container.innerHTML = `
          <div class="vg-wrap">
            <div class="vg-hud">
              ...
              <div class="sd-pill"><div class="l">체인</div><div class="v" id="wc-chainlen">${chain.length}</div></div>
      ...
      function onValid(word, data) {
        ...
        showResult(`<span class="ok-msg">✓ ${word}</span>`);
      ...
      function onWrong(reason, input) {
        ...
        input.focus();
        renderHearts();
        if (lives <= 0) { gameOver(REASON_MSG[reason] || "오답!"); return; }
      ...
      function computeStats() {
        ...
        const score = chainLen * 100 + lives * 100;
        return { chainLen, elapsedS, avgS, score };
      }
      ...
      function gameOver(reasonText) {
        ...
        overlay(`
          <div class="big">💥</div>
          <h2>게임 종료</h2>
          <p class="bad-msg">목숨을 다 잃었어요 · ${reasonText}</p>
          ${statsHtml(stats)}
          <div class="row">
